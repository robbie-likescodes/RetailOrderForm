...
function updateOrderItemStatus_(payload, requestId) {
  ...
  const allowed = ["Unavailable", "Complete", "Not Complete", "Pulled", "Not Pulled"];
  const partialMatch = status.match(/^Partially Collected\s+\d+\s+of\s+\d+$/i);
  if (!allowed.includes(status) && !partialMatch) {
    return buildError_("Invalid status.", "INVALID_STATUS", { allowed: ["Unavailable", "Complete", "Not Complete", "Partially Collected X of Y"] }, requestId);
  }
